...
class Georectification:
    def __init__(self, config, parameter, dem, geoPose)                     -> None:
        """
        Initialize the Georectification object
        """
        self.config                = config
    ...
    def load_geopose(self, geoPose)                                         -> None:
        """
        Load the GeoPose object
        """
        self.geoPose = geoPose
    def load_dem(self, dem)                                                 -> None:
        self.dem = dem
    def calculate_uav_azimuth(self)                                         -> None:
        """
        ...
        if self.geoPose is None:
            print("No GeoPose object loaded! \n     -> Load GeoPose object first! \n     -> Use <Georectification>.load_geopose(<GeoPose>) \n     -> <Georectification>.synchronize_image_geopose() \n     -> <Georectification>.calculate_uav_azimuth()")
            return
    ...
    def georectify(self, idx, imageName, verbose=False)                     -> None:
        """"
    ...
    def georectify_all(self, verbose=False)                                 -> None:
        """
            ...
            if len(files) > 0:
                ...
                if overwrite == False:
                    ...
                    if user_input.lower() == 'n':
                        overwrite == False
                        print("Georectification aborted!")
                        return
                    elif user_input.lower() == 'y':
                        overwrite == True
                    else:
                        print("Invalid input! Georectification aborted!")
                        return
...
#        self.image_coordinates_utm = self._convert_ECEF_to_UTM(self.geoPose.p_eg_e, self.epsg_code)
        # Convert the ECEF coordinates to UTM coordinates (self.image_coordinates_utm)
        self._convert_ECEF_to_UTM()
    ...
    PRIVATE METHODS
    """
    def _calc_uav_azimuth_gnd_trk(self, idx, north_vector=np.array([0, 1])) -> float:
        """ 
        ...
        if np.cross(flight_direction, north_vector) < 0:
            azimuth = -azimuth
        return azimuth
    def _convert_ECEF_to_UTM(self)                                          -> None:
        """
        Convert ECEF coordinates to UTM coordinates
        """

        epgs_string_to   = f"epsg:{self.epsg_code}"
        epgs_string_from = f"epsg:{c.EPSG_geocent}"
        wgs84_to_utm     = Transformer.from_crs(epgs_string_from, epgs_string_to, always_xy=True)

        utm_coord = np.zeros_like(self.geoPose.p_eg_e)

        if len(self.geoPose.p_eg_e.shape) == 1:
            # One dimensional array
            easting, northing, alt = wgs84_to_utm.transform(self.geoPose.p_eg_e[0], self.geoPose.p_eg_e[1], self.geoPose.p_eg_e[2])
            utm_coord              = [easting, northing, alt]
        if len(self.geoPose.p_eg_e.shape) == 2:
            # Two dimensional array
            for meas_i in range(self.geoPose.p_eg_e.shape[0]):
                easting, northing, alt = wgs84_to_utm.transform(self.geoPose.p_eg_e[meas_i, 0], self.geoPose.p_eg_e[meas_i, 1], self.geoPose.p_eg_e[meas_i, 2])
                utm_coord[meas_i, :]   = [easting, northing, alt]
        elif len(self.geoPose.p_eg_e.shape) == 3:
            # Three dimensional array
            for meas_i in range(self.geoPose.p_eg_e.shape[0]):
                for vec_i in range(self.geoPose.p_eg_e.shape[1]):
                    easting, northing, alt      = wgs84_to_utm.transform(self.geoPose.p_eg_e[meas_i, vec_i, 0], self.geoPose.p_eg_e[meas_i, vec_i, 1], self.geoPose.p_eg_e[meas_i, vec_i, 2])
                    utm_coord[meas_i, vec_i, :] = [easting, northing, alt]
        else:
            print("ERROR: ECEF coordinates have wrong shape!")
            return

        self.image_coordinates_utm = utm_coord

    """
    STATIC METHODS
    """
    @staticmethod
    def _downscale_image(image_data, downscale_factor)                      -> gdal.Dataset:
        """
    ...
    @staticmethod
    def _calculate_geotransform(image_corners, uav_azimuth, img_azim, width_pixel, height_pixel, tuning_values=None) -> tuple:
        """
    ...
    @staticmethod
    def _calculate_image_rotation(coordinates, northing_ref_vect=np.array([0, 1]), debug=False)                      -> tuple:
        """
...
#            left_azim = -left_azim
    ...
    @staticmethod
    def _mirror_image(I, axis)                                                                                       -> np.ndarray:
        """
