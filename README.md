...
## Orthorectification
The orthorectification step is voluntary. The flag `orthorectify_images` in the config file switches "on" or "off" orthorectification. Orthorectification is a computationally heavy process and may improve the overlap missalignment only marginally.
The Orthorectification step does NOT improve georeferencing. It aims to reduce the overlap missalignment between neighbouring images. There are 4 different methods which are tried.
- Feature matching: This method tries to identify features in the whole image and match them with features in the neighbouring images.
- Optical flow: This method tries to calculate which direction certain "pixels" have traveled in the overlaping region of the image (only the overlapping region + a frame around this is considered for this approach)
- Phase correlation: This is a frequency-domain approach, trying to match the phase of overlapping images (only the overlap is considered)
- Normalized Cross-Correlation (NCC): This method works in the spatial domain and tries to maximize the cross correlation between neighbouring images (only the overlap is taken into account for this method).
...
|<g>orthorectify_images</g>|`'boolean'`|If set `'True'`, the images will be orthorectified after the georeferencing (this might be computationally expensive)|
|<g>orthorectify_images</g>|`'boolean'`|If set `'True'`, the images will be orthorectified after the georeferencing (this might be computationally expensive)|
|<g>image_matching_methods</g>|`'string'`|This sets which imaging matching methods shall be used. Choices are `'auto'`, `'featurebased'`, `'phasecorrelation'`, `'opticalflow'`, `'ncc'`. The method chosen is the one used to match images. if `'auto'` is chosen a function to determine the best method for each image pair is used. If `'use_fallback_methods'` is used, a different fallback method than the one chosen might be used.|
|<g>use_fallback_methods</g>|`'boolean'`|Flag to determine if a fallback method (one of `'featurebased'`, `'phasecorrelation'`, `'opticalflow'`, `'ncc'`) shall be used in case the primary method does not yield good enough results.|
|<g>orthorectification_band</g>|`'int'`|Image band which shall be used for orthorectification. First band = 0|
|<g>FeatureMatching_methods</g>|`'string'`|This sets which matching method shall be used when the orthorectification method  `'featurebased'` is used. Options are:`'sift'`, `'orb'`, `'akaze'`, `'brisk'`, `'fast'`, `'kaze'`, `'brief'` or a combination. E.g. ['sift', 'orb', 'akaze']|
|<g>feature_weight</g>|`'int'`|Weight given to the method feature-matching  (can be between 0 and 1)|
|<g>opticalflow_weight</g>|`'int'`|Weight given to the method optical flow  (can be between 0 and 1)|
|<g>phasecorrelation_weight</g>|`'int'`|Weight given to the method phase-correlation  (can be between 0 and 1)|
|<g>ncc_weight</g>|`'int'`|Weight given to the method NCC (can be between 0 and 1)|
|<g>only_transl_rotation</g>|`'boolean'`|Flag allowing to choose whether images should be tried to be matched by only tranlation and rotation (`'True'`) or translation, rotation, skew and scaling (`'False'`)|
|<g>maximal_allowed_trans_meter</g>|`'float'`|Maximum tranlation allowed [`'meter'`]|
|<g>maximal_allowed_rot_deg</g>|`'float'`|Maximum rotation allowed for an image [`'degree'`]|
|<g>maximal_allowed_skew_deg</g>|`'float'`|Maximum skew allowed for an image (each image edge) [`'degree'`] (this is ineffective if `'only_transl_rotation = False`')|
|<g>maximal_allowed_scale_perc</g>|`'float'`|Maximum scaling of the image in `'x`' and `'y`' axis. [`'percent'`] (this is ineffective if `'only_transl_rotation = False`')|
...
graph TD
    ...
    output              --> ortho[Orthorectification <br> of the images]
...
class images,output,inpOut,geoLog,ortho io
